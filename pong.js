...
//set player one chop image as invisible
$('#playerOneChop1').toggle();
$('#playerOneChop2').toggle();
$('#playerOneChop3').toggle();
$('#playerOneChop4').toggle();
$('#playerOneChop5').toggle();
$('#playerOneChop6').toggle();
$('#playerOneChop7').toggle();
$('#playerOneChop8').toggle();
$('#playerOneChop9').toggle();
...
//=============================================================================
// PLAYER SELECTION + SIGN IN
//=============================================================================

  startDemo:         function() { this.start(0); },
  startSinglePlayer: function() { console.log("one player");
                                  //call playerZeroLogIn();
                                  //if enoughFriends0 == true, then:
                                      this.start(1);
                                  //if enoughFriends0 == false, then:
                                      //relaunch sign in window
                                },
  

  // startDoublePlayer: function() { console.log("two players"); 
  //                                 //call playerZeroLogIn();
  //                                 window.open("http://www.twitter.com","_blank","toolbar=yes, location=yes, directories=no, status=no, menubar=no, scrollbars=yes, resizable=no, width=740, height=740");
  //                                 //if enoughFriends0 == true, then:
  //                                     //call playerOneLogIn();
  //                                   //if enoughFriends0 == false, then:
  //                                     //window.alert("sorry, you need to follow at least 10 people to play");
  //                                     //relaunch sign in window
  //                                   //if yes,
  //                                     //call playerOneLogIn();
  //                                       //if enoughFriends1 == false, then:
  //                                         //window.alert("sorry, you need to follow at least 10 people to play");
  //                                         //relaunch sign in window
  //                                       //if enoughFriends1 == true, then: 
  //                                           this.start(2); 
  //                               },

//=============================================================================
// PLAYER SELECTION + SIGN IN END
//=============================================================================

  start: function(numPlayers) {
    if (!this.playing) {
      this.scores = [0, 0];  //set score
      this.playing = true;  //activate ball
  ...
  stop: function(ask) {
    if (this.playing) {
      if (!ask || this.runner.confirm('Seriously? You give up?')) {
        this.playing = false;
...
//=============================================================================
  ...
  goal: function(playerNo) {
    ...
    ///////////////////////////////
    /////add player one functions
    ///////////////////////////////

      //create player one unfriend function
      function playerOneUnfriend() {
        var request = new XMLHttpRequest();
        request.open('GET', 'http://www.helloimalex.com/mstwo_pong/playerOne/unfriend.php', false);
        request.send();

          $.ajax({
          type: 'POST',    // take an action (can be post or get)
          url: 'playerOne/unfriend.php' // call unFriend script
          // when the server responds
          }).done(function(response) {
            //print this
          console.log("one less friend! that sucks.");
        });
      }

      //create chopping block friend blur
      function playerOneCrossOut(){
        playerOneCounter++;
        if(playerOneCounter == 1){
          $('#playerOneChop1').toggle();
          $('#playerOneHolder1').toggle();
        }
        if(playerOneCounter == 2){
          $('#playerOneChop2').toggle();
          $('#playerOneHolder2').toggle();
        }
        if(playerOneCounter == 3){
          $('#playerOneChop3').toggle();
          $('#playerOneHolder3').toggle();
        }
        if(playerOneCounter == 4){
          $('#playerOneChop4').toggle();
          $('#playerOneHolder4').toggle();
        }
        if(playerOneCounter == 5){
          $('#playerOneChop5').toggle();
          $('#playerOneHolder5').toggle();
        }
        if(playerOneCounter == 6){
          $('#playerOneChop6').toggle();
          $('#playerOneHolder6').toggle();
        }
        if(playerOneCounter == 7){
          $('#playerOneChop7').toggle();
          $('#playerOneHolder7').toggle();
        }
        if(playerOneCounter == 8){
          $('#playerOneChop8').toggle();
          $('#playerOneHolder8').toggle();
        }
        if(playerOneCounter == 9){
          $('#playerOneChop9').toggle();
          $('#playerOneHolder9').toggle();
          playerOneCounter = 0;
        }
      }
   
    ///////////////////////////////
    /////add player one goal events
    ///////////////////////////////
    
    if(playerNo == 1){
      //playerOneUnfriend();
      playerOneCrossOut();
      console.log("LEFT UNFRIEND");    
      }
    ...
    ///////////////////////////////
    /////add player two goal events
    ///////////////////////////////

   if(playerNo == 0){
      // playerTwoCrossOut();
      console.log("RIGHT UNFRIEND");
    }

    ///////////////////////////////
    /////stop game at nine points
    ///////////////////////////////
...
//=============================================================================
  ...
  onkeydown: function(keyCode) {
    switch(keyCode) {
      ...
      case Game.KEY.UP:    if (!this.leftPaddle.auto)  this.leftPaddle.moveUp();    break;
      case Game.KEY.DOWN:    if (!this.leftPaddle.auto)  this.leftPaddle.moveDown();  break;
  ...
  onkeyup: function(keyCode) {
    switch(keyCode) {
      case Game.KEY.UP: if (!this.leftPaddle.auto)  this.leftPaddle.stopMovingUp();    break;
      case Game.KEY.DOWN: if (!this.leftPaddle.auto)  this.leftPaddle.stopMovingDown();  break;
